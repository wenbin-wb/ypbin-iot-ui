 ...
 * 右侧地址/周期/缩放是这台设备自己的。两边靠 `point.propertyId === property.id` 关联，
 * 未映射的属性用橙色标签标出（用户一眼就知道「这台设备还差哪些点位没配」）。
 *
 * 数据来源全部既有：`GET /iot/devices/{id}/points` + `GET /iot/products/{id}/services`
 * + `.../services/{serviceId}/properties`。属性必须在服务下逐服务取（后端层级如此），
 * 故这里对「该产品的服务」做一次 `Promise.all` 扇出——服务数量是个位数，
 * 且**与设备数量无关**（不存在按设备循环的 N+1）。
 *
 * 本轮只读展示：点位映射的增删改接口已就绪，但做成半个表单不如不做（未映射项已明确标出）。
 */
...
const groups = ref<
  { properties: IotThingModelApi.PropertyResp[]; service: IotThingModelApi.ServiceResp }[]
>([]);
...
/** 属性 ↔ 点位 的连接结果（属性为左表主键，未配点位的属性 `point` 为空）。 */
  ...
  for (const group of groups.value) {
    for (const property of group.properties) {
      rows.push({
        ...
        point: byProperty.get(property.id),
...
<template>
  <Spin :spinning="loading">
    ...
    <template v-if="orphanPoints.length > 0">
      ...
      <Alert :message="$t('page.iot.point.orphanHint')" show-icon type="warning" />
